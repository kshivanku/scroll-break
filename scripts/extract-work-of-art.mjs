...
const inputPath = "assets/workofart.pdf";
const outputPath = "assets/work-of-art.txt";
...
function extractPositionedText(pdf) {
  ...
  while ((match = streamRegex.exec(bytes))) {
    ...
    if (!/FlateDecode/.test(streamHead)) continue;
    ...
    if (!/\bBT\b/.test(content) || !/\bTj\b/.test(content)) continue;
...
function normalizeBody(lines) {
  ...
  for (const line of lines) {
    ...
    if (!text || text === "*" || /^Quoted from Paul Valery/.test(text) || /Manheim, p\. 225/.test(text)) continue;
    ...
    if (hasLargeGap && current && /[.!?:]"?$/.test(current)) {
      flush();
    }

    if (!current) current = text;
    else if (current.endsWith("-")) current = `${current.slice(0, -1)}${text}`;
  ...
  return paragraphs
    .join("\n\n")
    .replace(/-\n\n(?=[a-z])/g, "")
    .replace(/\bPIeCES SUR L'ART\b/g, "Pieces sur l'Art")
    .replace(/\bValery\b/g, "Valery")
    .trim();
}

const pages = extractPositionedText(await readFile(inputPath));
const cleanedText = `${normalizeBody(pages.flat())}\n`;
